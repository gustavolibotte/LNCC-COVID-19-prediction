...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 24 10:44:27 2021
...
# Locations
locations = open("init_modelsIN.txt", "r").read().split("\n")[:-1]
loc_params = []
loc_y0 = []

for i in range(len(locations)):
    
    params = [np.float(p) for p in locations[i].split(" ")[1:]]
    locations[i] = locations[i].split(" ")[0]
    
    init_model = getattr(epi_mod, locations[0])
    loc_params.append(params[:init_model.nparams])
    loc_y0.append(params[init_model.nparams:])
    ...
    for i in range(1, repeat+1):
        ...
        for j in range(len(locations)):
            ...
            if (day_step == 0):
                ...
                n_sets = int((data_length-day_set_size-val_set_size) // day_step)
...
for i in range(len(locations)):
    ...
    if (rank == root):
        ...
        log_geral.write("Location: "+locations[i]+", Parameters:"+str(loc_params[i])+", y0 = "+str(loc_y0[i])+"\n\n")
    ...
    for j in range(len(models)):
        ...
        init_model = getattr(epi_mod, locations[i])
        
        # Get data
        data = init_model.infected_dead(t, np.array(loc_params[i]), np.array(loc_y0[i]))
        
        data = pd.DataFrame({"day":t, "confirmed":data[0], "dead":data[1]})
        ...
        for days_idx in range(start_idx, len(days_sets)):
            ...
            if (days_folders.index(filepath.split("/")[-1]) == 0 or use_last_post == False):
                ...
                if (rank == root):
                    ...
                    eps = np.percentile(samples, 100*n/n_max)
                    wait_var = 0
