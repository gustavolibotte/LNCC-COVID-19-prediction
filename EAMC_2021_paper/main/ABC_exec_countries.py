...
repeat = 20 # Number of posteriors to be calculated
...
df_countries = pd.read_csv(r"covid_19_clean_complete.csv")
...
for i in range(len(locations)):
    ...
    for j in range(len(models)):
        ...
        # Get data
        data = df_countries[df_countries["Country/Region"] == locations[i]]
        provinces = np.unique(data["Province/State"])
        
        if (type(provinces[0]) == float):
            ...
            for k in range(len(dates)):
                conf_dead.append(np.float64(np.sum(data[data["Date"] == dates[k]][["Confirmed", "Deaths"]])))
            
            data = pd.DataFrame(np.concatenate(conf_dead).reshape((len(conf_dead), 2)),
